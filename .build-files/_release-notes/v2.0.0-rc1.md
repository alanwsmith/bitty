...
- TODO: Added `data-functions` which loads
either a hash with the given name on the
window object or a module with that
contains funcions and pulls them 
all into so they can be used with 
`this.api.fn.FUNCTION()`. bitty
also checks to see if there's a
`window.bittyFunctions` variable
on the window and loads it automatically.

This provides a plugin style way
to add functions that can be
reused across multiple bitty
components
