...
- TODO: Send `error(message)` if a send/receive function
doesn't exist.
- TODO: Added more generic error handing where you just send
a message and it outputs to the page along with the
UUID of the element that had the problem. The 
output elements have `bitty-js-error` classes on them
so their display can be controlled.  
- TODO: Update to use data-connection instead of data-connection as the
name
- TODO: Pull in local classes from a global bittyClasses object
if they exist
